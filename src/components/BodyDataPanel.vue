...
const patternmakerRiskSummary = computed(() => {
  const nextShootText = nextShoot.value
    ? `${nextShoot.value.name}需达到“${nextShoot.value.requiredDressState}”`
    : '暂无待拍摄节点'
  const pendingRounds = fittingRounds.value.filter((round) => round.status !== 'completed')
  const pendingChanges = fittingRounds.value.flatMap((round) => round.changes).filter((change) => change.status !== 'completed')
  const canCatchWedding = brideStore.daysUntilWedding > 7 && pendingRounds.length <= 2

  return {
    nextShootText,
    pendingRoundText: pendingRounds.length > 0
      ? pendingRounds.map((round) => `${round.title}第${round.roundNumber}轮`).join('、')
      : '所有试衣轮次已完成',
    pendingChangeText: pendingChanges.length > 0
      ? pendingChanges.map((change) => `${change.description}(${change.estimatedDays}天)`).join('、')
      : '暂无未完成修改项',
    canCatchWedding,
    deliveryText: canCatchWedding ? '当前节奏可覆盖婚礼交付窗口' : '交付窗口紧张，需要压缩修改或加急确认',
  }
})
</script>
...
<template>
  <div class="card-elegant p-6">
    ...
    <div v-if="isPatternView" class="mt-4 p-3 rounded-elegant border border-mist-purple/20 bg-mist-purple/10">
      <div class="flex items-center justify-between gap-3">
        <div class="flex items-center gap-2">
          <AlertCircle
            v-if="!patternmakerRiskSummary.canCatchWedding"
            class="w-4 h-4 text-rose-gold"
          />
          <Shield v-else class="w-4 h-4 text-emerald-600" />
          <span class="text-sm font-medium text-espresso">版师赶期判断</span>
        </div>
        <span
          class="rounded-full px-2 py-0.5 text-xs font-medium"
          :class="patternmakerRiskSummary.canCatchWedding ? 'bg-emerald-500/15 text-emerald-600' : 'bg-rose-gold/15 text-rose-gold'"
        >
          {{ patternmakerRiskSummary.canCatchWedding ? '可赶上婚礼' : '存在延期风险' }}
        </span>
      </div>
      <div class="mt-3 grid grid-cols-1 gap-2 text-xs leading-relaxed text-espresso/65">
        <p class="rounded-lg bg-white/70 px-2.5 py-2">拍摄节点：{{ patternmakerRiskSummary.nextShootText }}</p>
        <p class="rounded-lg bg-white/70 px-2.5 py-2">修改轮次：{{ patternmakerRiskSummary.pendingRoundText }}</p>
        <p class="rounded-lg bg-white/70 px-2.5 py-2">修改工期：{{ patternmakerRiskSummary.pendingChangeText }}</p>
        <p class="rounded-lg bg-white/70 px-2.5 py-2">交付判断：{{ patternmakerRiskSummary.deliveryText }}</p>
      </div>
    </div>
